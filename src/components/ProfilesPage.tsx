    ...
    return <article key={key} {...reorder.cardProps(owner.id, profile.id)} className={cn("profile-row", isDefault && "default", selectMode && "selecting", isSelected && "selected", isDragging && "dragging", isMenuOpen && "menu-open")}
      ...
      <div className="profile-row-specs">
        ...
        <div className="profile-flags compact">
          <span className={profile.flashAttention ? "enabled" : ""}>FA</span>
          {profile.jinja && <span className="enabled">Jinja</span>}
          {profile.cacheTypeK !== "f32" && <span>{profile.cacheTypeK}</span>}
          {profile.reasoning === "on" && profile.reasoningEffort !== "auto" && <span className="enabled">{profile.reasoningEffort}</span>}
        </div>
      </div>
