...
interface Props {
  ...
  onUpdateModelTags: (modelId: string, quantization: string, tags: string[], nextPool?: string[]) => void;
...
export default function ModelsPage(props: Props) {
  ...
  const [tagModalModel, setTagModalModel] = useState<ModelAsset | null>(null);
  ...
  return <>
    ...
    {tagModalModel && (
      <ModelTagModal
        model={tagModalModel}
        tagPool={props.config.customTags}
        onClose={() => setTagModalModel(null)}
        onSave={(quant, tags, nextPool) => {
          props.onUpdateModelTags(tagModalModel.id, quant, tags, nextPool);
          setTagModalModel(null);
        }}
      />
    )}
  </>;
...
interface ModelItemProps {
  ...
  onRenameModel: (modelId: string, displayName: string) => void; onOpenTagModal: () => void; onSetDefaultModel: (modelId: string) => void; onRemove: () => void;
  selectMode: boolean; isSelected: boolean; isJustImported: boolean; isDragging: boolean; cardHandlers: CardHandlers;
  onToggleSelect: () => void;
}

function ModelCard({ model, profiles, selectedProfileId, isDefaultModel, isRunning, busy, menuOpen, onMenu, onSelectProfile, onStart, onStop, onEditProfile, onAddProfile, onRenameModel, onOpenTagModal, onSetDefaultModel, onRemove, selectMode, isSelected, isJustImported, isDragging, cardHandlers, onToggleSelect }: ModelItemProps) {
  const { t } = useI18n();
  ...
  return <article {...cardHandlers} className={cn("model-card", isDefaultModel && "default", isRunning && "running", selectMode && "selecting", isSelected && "selected", isDragging && "dragging", menuOpen && "menu-open")}
    ...
    <div className="model-tags">
      ...
      <span
        className={cn("quant-tag", isUnknownQuant && "unknown-quant")}
        title={isUnknownQuant ? t("card.manageTags") : t("card.dblClickToEditQuant")}
        onDoubleClick={(e) => {
          e.stopPropagation();
          onOpenTagModal();
        }}
...
function ModelRow({ model, profiles, selectedProfileId, isDefaultModel, isRunning, busy, menuOpen, onMenu, onSelectProfile, onStart, onStop, onEditProfile, onAddProfile, onRenameModel, onOpenTagModal, onSetDefaultModel, onRemove, selectMode, isSelected, isJustImported, isDragging, cardHandlers, onToggleSelect }: ModelItemProps) {
  const { t } = useI18n();
  const [renaming, setRenaming] = useState(false);
  const [draftName, setDraftName] = useState("");
  const selected = profiles.find((profile) => profile.id === selectedProfileId) || profiles[0];
